...
function getAssetsDir() {
  return (0, import_node_path.join)(__dirname, "..", "assets");
}
var AI_TO_PLATFORM = {
  claude: "claude",
  cursor: "cursor",
...
async function loadPlatformConfig(aiType) {
  ...
  const configPath = (0, import_node_path.join)(getAssetsDir(), "templates", "platforms", `${platformName}.json`);
  const content = await (0, import_promises.readFile)(configPath, "utf-8");
  return JSON.parse(content);
}
async function loadTemplate(templateName) {
  const templatePath = (0, import_node_path.join)(getAssetsDir(), "templates", templateName);
  return (0, import_promises.readFile)(templatePath, "utf-8");
}
...
async function copyTaiwanInvoiceAssets(targetSkillDir, sections) {
  const sourceDir = (0, import_node_path.join)(getAssetsDir(), "taiwan-invoice");
  if (sections.examples && await exists((0, import_node_path.join)(sourceDir, "EXAMPLES.md"))) {
    await (0, import_promises.cp)((0, import_node_path.join)(sourceDir, "EXAMPLES.md"), (0, import_node_path.join)(targetSkillDir, "EXAMPLES.md"));
  }
  if (sections.references && await exists((0, import_node_path.join)(sourceDir, "references"))) {
    const refsTarget = (0, import_node_path.join)(targetSkillDir, "references");
    await (0, import_promises.mkdir)(refsTarget, { recursive: true });
    await (0, import_promises.cp)((0, import_node_path.join)(sourceDir, "references"), refsTarget, { recursive: true });
  }
  if (sections.scripts && await exists((0, import_node_path.join)(sourceDir, "scripts"))) {
    const scriptsTarget = (0, import_node_path.join)(targetSkillDir, "scripts");
...
function detectAIType(cwd = process.cwd()) {
  const detected = [];
  if ((0, import_node_fs.existsSync)((0, import_node_path2.join)(cwd, ".claude"))) {
...
var logger = {
  ...
  title: (msg) => console.log(import_chalk.default.bold.cyan(`
${msg}
`)),
...
async function initCommand(options) {
  ...
  if (!aiType) {
    ...
    if (detected.length > 0) {
      logger.info(`Detected: ${detected.map((t) => import_chalk2.default.cyan(t)).join(", ")}`);
    }
  ...
  try {
    ...
    copiedFolders.forEach((folder) => {
      console.log(`  ${import_chalk2.default.green("+")} ${folder}`);
    });
    console.log();
    logger.success("Taiwan Invoice Skill installed successfully!");
    console.log();
    console.log(import_chalk2.default.bold("Next steps:"));
    console.log(import_chalk2.default.dim("  1. Restart your AI coding assistant"));
...
async function infoCommand() {
  ...
  console.log(import_chalk4.default.cyan("Skill Information:"));
  console.log();
  console.log(`  ${import_chalk4.default.dim("Name:")}        taiwan-invoice`);
  ...
  console.log(`  ${import_chalk4.default.dim("GitHub:")}      https://github.com/Moksa1123/taiwan-invoice`);
  console.log(`  ${import_chalk4.default.dim("npm:")}         https://www.npmjs.com/package/taiwan-invoice-skill`);
  console.log(`  ${import_chalk4.default.dim("Support:")}     https://paypal.me/cccsubcom`);
  console.log();
}
