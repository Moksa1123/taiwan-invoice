...
export async function updateCommand(): Promise<void> {
  logger.title('Taiwan Payment Skill - Update');
  ...
  try {
    const response = await fetch('https://registry.npmjs.org/taiwan-payment-skill/latest', {
      headers: {
    ...
    if (latestVersion === VERSION) {
      ...
      console.log(chalk.dim('  npm install -g taiwan-payment-skill@latest'));
    }
  } catch (error) {
