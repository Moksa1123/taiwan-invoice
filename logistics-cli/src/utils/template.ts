...
async function copyTaiwanLogisticsAssets(targetSkillDir: string, sections: PlatformConfig['sections']): Promise<void> {
  const sourceDir = join(getAssetsDir(), 'taiwan-logistics');
...
export async function renderWorkflowFile(config: PlatformConfig): Promise<string> {
  const frontmatter = renderFrontmatter(config.frontmatter);
  const sharedPath = config.folderStructure.sharedPath || 'taiwan-logistics';
...
export async function generatePlatformFiles(
  ...
  if (isGlobal) {
    ...
    // - Antigravity: ~/.gemini/antigravity/global_skills/taiwan-logistics/
    // The skillPath is like "skills/taiwan-logistics", we use it directly for most platforms
    // But for Antigravity global_skills, we only need "taiwan-logistics"
    ...
    // Project installation: targetDir/.cursor/skills/taiwan-logistics/
    skillDir = join(
      targetDir,
  ...
  await copyTaiwanLogisticsAssets(skillDir, config.sections);
